...
RSpec.describe ConvertFile::InpToHash, type: :service do
  describe ".call" do
    context "when convert inp files to hash sucessful" do
      ...
      it "returns the number of arrays generated by all files" do
        user_submission = create(:user_submission, :all_files)

        result = described_class.call(user_submission.files.first, user_submission).size +
                 described_class.call(user_submission.files.second, user_submission).size +
                 described_class.call(user_submission.files.last, user_submission).size

        expect(result).to eq 916
      end
